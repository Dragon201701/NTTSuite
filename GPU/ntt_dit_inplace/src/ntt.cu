...
__global__ void butter_prec(DATA_TYPE *result, DATA_TYPE *twiddle, DATA_TYPE p, unsigned k, unsigned mid, unsigned i) {
    int j = blockIdx.x * blockDim.x + threadIdx.x;
    if(j < mid){
        DATA_TYPE w = twiddle[(1 << (VECTOR_ADDR_BIT - i)) * j];
        DATA_TYPE factor1   = result[j + k];
        DATA_TYPE factor2   ;
        modulo_cu(w * result[j + k + mid], p, factor2);
        modulo_cu(factor1 + factor2, p, result[j + k]);
        modulo_cu(factor1 - factor2, p, result[j + k + mid]);
    }
}
...
DATA_TYPE* ntt_flat(DATA_TYPE * vec,  int n, DATA_TYPE p, DATA_TYPE g){
	...
	for(unsigned i = 1; i <= t; i++){ 
        ...
        for(unsigned  k = 0; k < n; k+=m){
            butter_prec<<<blocksPerGrid, threadsPerBlock>>>(d_y, d_twiddle, p, k, mid, i);
		}
    }
