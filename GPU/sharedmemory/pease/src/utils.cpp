...
void randVec(DATA_TYPE n, DATA_TYPE * vec, DATA_TYPE max){
...
DATA_TYPE* twiddle_cal(DATA_TYPE n, DATA_TYPE r, DATA_TYPE p) {
	...
	for (int i = 1; i <= log2(n); i++) {
		//uint64_t base = pow(r, (p - 1) / pow(2, i));
		DATA_TYPE base = modExp(r, (p - 1) / pow(2, i), p);
		for (int k = 0; k < pow(2, i - 1); k++) {
			twiddle[(int)pow(2, i - 1) - 1 + k] = modExp(base, k, p);
		}
	}
	return twiddle;
}

void twiddle_cal(DATA_TYPE n, DATA_TYPE r, DATA_TYPE p, DATA_TYPE * twiddle) {
	for (int i = 1; i <= log2(n); i++) {
		//uint64_t base = pow(r, (p - 1) / pow(2, i));
		DATA_TYPE base = modExp(r, (p - 1) / pow(2, i), p);
		for (int k = 0; k < pow(2, i - 1); k++) {
