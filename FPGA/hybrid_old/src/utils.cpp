 ...
 * @param revArr The vector to save bit reverse num
 * @return    The bit reversed vector
 */
void getBitReverseArr(DATA_TYPE n, DATA_TYPE *revArr){

	DATA_TYPE num_bits = log2(n);

	PARAMS_TYPE reverse_num;
	for(PARAMS_TYPE i = 0; i < n; i++){

		reverse_num = 0;
		for(PARAMS_TYPE j = 0; j < num_bits; j++){
