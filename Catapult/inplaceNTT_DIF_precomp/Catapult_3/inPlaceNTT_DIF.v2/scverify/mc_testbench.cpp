...
// Generated date: Mon Sep 06 21:30:19 EDT 2021
//
// ----------------------------------------------------------------------------
// 
// -------------------------------------
...
extern "C++" void inPlaceNTT_DIF( ac_int<32, false > vec[1024], ac_int<32, false > p, ac_int<32, false > r,  ac_int<32, false > twiddle[1024],  ac_int<32, false > twiddle_h[1024]);
#endif
#ifndef CCS_SCVERIFY_USE_CCS_BLOCK
void testbench::exec_inPlaceNTT_DIF( ac_int<32, false > vec[1024], ac_int<32, false > p, ac_int<32, false > r,  ac_int<32, false > twiddle[1024],  ac_int<32, false > twiddle_h[1024]) {
   return mc_testbench::exec_inPlaceNTT_DIF(vec, p, r, twiddle, twiddle_h);
}
#endif
...
void mc_testbench::capture_vec_IN( ac_int<32, false > vec[1024])
{
   ...
   if (_capture_vec_IN && !testbench::vec_ignore)
   {
      ...
      mgc_sysc_ver_array1D<ac_int<32, false >,1024> vec_IN_tmp;
...
void mc_testbench::capture_vec( ac_int<32, false > vec[1024])
{
   if (_capture_vec)
   {
      ...
      ++vec_iteration_count;
      mc_golden_info< mgc_sysc_ver_array1D<ac_int<32, false >,1024>, MaskPacket<0, 32> > vec_tmp(testbench::vec_ignore, false, vec_iteration_count);
      vec_tmp._data.mc_testbench_process_array_bounds("vec",testbench::vec_array_comp_first,testbench::vec_array_comp_last,1023,0);
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_vec_IN->ok_to_put() | ccs_p->ok_to_put() | ccs_r->ok_to_put() | ccs_twiddle->ok_to_put() | ccs_twiddle_h->ok_to_put());
      else
         wait(ccs_vec_IN->ok_to_put() | ccs_p->ok_to_put() | ccs_r->ok_to_put() | ccs_twiddle->ok_to_put() | ccs_twiddle_h->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN( ac_int<32, false > vec[1024], ac_int<32, false > p, ac_int<32, false > r,  ac_int<32, false > twiddle[1024],  ac_int<32, false > twiddle_h[1024])
{
...
void mc_testbench::capture_OUT( ac_int<32, false > vec[1024], ac_int<32, false > p, ac_int<32, false > r,  ac_int<32, false > twiddle[1024],  ac_int<32, false > twiddle_h[1024])
{
...
void mc_testbench::exec_inPlaceNTT_DIF( ac_int<32, false > vec[1024], ac_int<32, false > p, ac_int<32, false > r,  ac_int<32, false > twiddle[1024],  ac_int<32, false > twiddle_h[1024])
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(vec, p, r, twiddle, twiddle_h);
   inPlaceNTT_DIF(vec, p, r, twiddle, twiddle_h);
   ...
   capture_OUT(vec, p, r, twiddle, twiddle_h);
   #else
   #endif
}
// ============================================
