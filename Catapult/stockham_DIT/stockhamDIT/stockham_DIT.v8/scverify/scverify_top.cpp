...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 15, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", 135.000000, false)
    , TLS_rst("TLS_rst")
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 285716, 285716, 0)
    , catapult_monitor_INST("Monitor", clk, true, 285716LL, 285716LL)
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(135.000000 , SC_NS);
  } else {
    if (toggle_hw_reset) {
