...
#define ccs_intercept_axi_test_4 \
  ccs_real_axi_test(ac_sync &run,ac_int<32, false > a[16],ac_sync &complete);\
  void axi_test(ac_sync &run,ac_int<32, false > a[16],ac_sync &complete)\
{\
    ...
    mc_testbench_capture_IN(run,a,complete);\
    ccs_real_axi_test(run,a,complete);\
    mc_testbench_capture_OUT(run,a,complete);\
}\
  void ccs_real_axi_test
#else
#define ccs_intercept_axi_test_4 axi_test
