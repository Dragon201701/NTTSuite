...
set data_inputs [list {vec_rsc_q[*]} {p_rsc_dat[*]} {r_rsc_dat[*]} {result_rsc_q[*]} {twiddle_rsc_q[*]}]
...
## Multi-cycle component nangate-45nm_beh.mgc_rem (IDX_LOOP_IDX_LOOP_rem_1_cmp) [cycles: 50]
set_multicycle_path 50 -through [get_pins {{DIT_RELOOP_core_inst/IDX_LOOP_IDX_LOOP_rem_1_cmp/z[*]}}]
...
set_output_delay -clock [get_clocks {clk}] 0.01 [get_ports {result_rsc_radr[*]}]
set_output_delay -clock [get_clocks {clk}] 0.01 [get_ports {result_rsc_re}]
set_input_delay -clock [get_clocks {clk}] 0.5 [get_ports {result_rsc_q[*]}]
set_output_delay -clock [get_clocks {clk}] 0.01 [get_ports {result_rsc_wadr[*]}]
set_max_delay 10.0 -from [all_inputs] -to [all_outputs]
set_output_delay -clock [get_clocks {clk}] 0.01 [get_ports {result_rsc_d[*]}]
set_output_delay -clock [get_clocks {clk}] 0.01 [get_ports {result_rsc_we}]
