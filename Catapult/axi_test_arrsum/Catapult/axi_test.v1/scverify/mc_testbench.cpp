...
// Generated date: Sun Jan 23 14:08:07 EST 2022
//
// ----------------------------------------------------------------------------
// 
// -------------------------------------
...
bool testbench::arrsum_ignore;
void mc_testbench_arrsum_skip(bool v) { testbench::arrsum_skip = v; }
bool testbench::arrsum_skip;
bool testbench::arrsum_skip_quiet;
bool testbench::arrsum_skip_once;
bool testbench::arrsum_skip_noerr;
int  testbench::arrsum_array_comp_first;
int  testbench::arrsum_array_comp_last;
bool testbench::arrsum_use_mask;
ac_int<32, false > testbench::arrsum_output_mask;
mc_wait_ctrl testbench::arrsum_wait_ctrl;
...
extern "C++" void axi_test( ac_sync &run,  ac_int<32, false > a[16],  ac_int<32, false > &arrsum,  ac_sync &complete);
#endif
#ifndef CCS_SCVERIFY_USE_CCS_BLOCK
void testbench::exec_axi_test( ac_sync &run,  ac_int<32, false > a[16],  ac_int<32, false > &arrsum,  ac_sync &complete) {
   return mc_testbench::exec_axi_test(run, a, arrsum, complete);
}
#endif
...
// ============================================
// Function: capture_arrsum
// --------------------------------------------

void mc_testbench::capture_arrsum( ac_int<32, false > &arrsum)
{
   if (_capture_arrsum)
   {
      int cur_iter = arrsum_iteration_count;
      ++arrsum_iteration_count;
      mc_golden_info< ac_int<32, false >, MaskPacket<0, 32> > arrsum_tmp(arrsum, testbench::arrsum_ignore, false, arrsum_iteration_count);
      // BEGIN: testbench output_mask control for field_name arrsum
      if ( testbench::arrsum_use_mask ) {
         sc_lv<32> tmp_mask_lv;
         type_to_vector(testbench::arrsum_output_mask, 32, tmp_mask_lv);
         arrsum_tmp._use_mask = true;
         arrsum_tmp._packet._mask = tmp_mask_lv;
      }
      // END: testbench output_mask control for field_name arrsum
      if (!testbench::arrsum_skip) {
         arrsum_golden.put(arrsum_tmp);
         ++arrsum_capture_count;
      } else {
         if (!testbench::arrsum_skip_quiet || !testbench::arrsum_skip_once) {
            std::ostringstream msg; msg.str("");
            msg << "testbench::arrsum_skip=true for iteration=" << arrsum_iteration_count << " @ " << sc_time_stamp();
            SC_REPORT_WARNING("User testbench", msg.str().c_str());
            testbench::arrsum_skip_once = true;
         }
      }
      mc_testbench_util::process_wait_ctrl("arrsum",testbench::arrsum_wait_ctrl,ccs_wait_ctrl_arrsum.operator->(),cur_iter,arrsum_capture_count,0);
      testbench::arrsum_use_mask = false;
   }
   testbench::arrsum_ignore = false;
   testbench::arrsum_skip = false;
}
// ============================================
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_run->ok_to_put() | ccs_a->ok_to_put());
      else
         wait(ccs_run->ok_to_put() | ccs_a->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN( ac_sync &run,  ac_int<32, false > a[16],  ac_int<32, false > &arrsum,  ac_sync &complete)
{
...
void mc_testbench::capture_OUT( ac_sync &run,  ac_int<32, false > a[16],  ac_int<32, false > &arrsum,  ac_sync &complete)
{
...
void mc_testbench::exec_axi_test( ac_sync &run,  ac_int<32, false > a[16],  ac_int<32, false > &arrsum,  ac_sync &complete)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(run, a, arrsum, complete);
   axi_test(run, a, arrsum, complete);
   ...
   capture_OUT(run, a, arrsum, complete);
   #else
   #endif
}
// ============================================
...
void mc_testbench::check_results()
{
   ...
   if (!_capture_arrsum) {
      cout<<"'arrsum' - warning, output was optimized away"<<endl;
   } else {
      _num_outputs_checked++;
      _failed |= arrsum_comp->check_results(arrsum_capture_count,testbench::arrsum_skip_noerr);
      _has_capture_counts |= !!(arrsum_capture_count);
   }
   if (!_capture_complete) {
...
void mc_testbench::wait_for_end() {
   ...
   if (arrsum_comp->get_compare_count() < arrsum_capture_count) {testbench_end_event.notify(1,SC_NS); return;}
...
void mc_testbench::run() {
   ...
   testbench::arrsum_ignore = false;
   testbench::arrsum_skip = false;
   testbench::arrsum_skip_quiet = false;
   testbench::arrsum_skip_once = false;
   testbench::arrsum_skip_noerr = false;
   testbench::arrsum_array_comp_first = -1;
   testbench::arrsum_array_comp_last = -1;
   testbench::arrsum_use_mask = false;
   testbench::arrsum_output_mask = (ac_int<32, false >) ~0;
   testbench::arrsum_wait_ctrl.clear();
   arrsum_capture_count = 0;
   arrsum_iteration_count = 0;
   testbench::complete_ignore = false;
   testbench::complete_skip = false;
   ...
   cout<<"   captured "<<arrsum_capture_count<<" values of arrsum"<<endl;
