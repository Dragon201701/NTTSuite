...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 52.63, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", 105.260002, false)
    , TLS_rst("TLS_rst")
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 27282, 27282, 0)
    , catapult_monitor_INST("Monitor", clk, true, 27282LL, 27282LL)
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(105.260002 , SC_NS);
  } else {
    if (toggle_hw_reset) {
