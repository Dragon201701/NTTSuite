...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 11.11, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", 122.209999, false)
    , TLS_rst("TLS_rst")
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 517142, 517142, 0)
    , catapult_monitor_INST("Monitor", clk, true, 517142LL, 517142LL)
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(122.209999 , SC_NS);
  } else {
    if (toggle_hw_reset) {
