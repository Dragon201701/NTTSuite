...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 11.76, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", 129.360001, false)
    , TLS_rst("TLS_rst")
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 493895, 493895, 0)
    , catapult_monitor_INST("Monitor", clk, true, 493895LL, 493895LL)
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(129.360001 , SC_NS);
  } else {
    if (toggle_hw_reset) {
