...
void stockham_dit(DATA_TYPE xt[VECTOR_WIDTH], DATA_TYPE p[VECTOR_WIDTH], DATA_TYPE twiddle[VECTOR_WIDTH], DATA_TYPE twiddle_h[VECTOR_WIDTH]){
    ...
    for(PARAMS_TYPE nn = 2; nn <= VECTOR_SIZE; nn = nn << 1, s = s >> 1, shift = shift - 1){
        ...
        for (int t = 0; t < (VECTOR_SIZE >> 1); t++) {
                    ...
                    x[(PARAMS_TYPE)(q + upper + s * m)] = modulo_sub(a - b, p);
        }
        DATA_TYPE *tmp = x;
        x = y;
        y = tmp;
