...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 5.56, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", 111.199997, false)
    , TLS_rst("TLS_rst")
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 15462, 15462, 0)
    , catapult_monitor_INST("Monitor", clk, true, 15462LL, 15462LL)
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(111.199997 , SC_NS);
  } else {
    if (toggle_hw_reset) {
