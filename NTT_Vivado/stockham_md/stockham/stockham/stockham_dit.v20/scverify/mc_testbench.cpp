...
// Generated date: Wed Sep 15 23:49:48 EDT 2021
//
// ----------------------------------------------------------------------------
// 
// -------------------------------------
...
extern "C++" void stockham_dit( ac_int<32, false > xt[16384], ac_int<32, false > p,  ac_int<32, false > twiddle[16384],  ac_int<32, false > twiddle_h[16384]);
#endif
#ifndef CCS_SCVERIFY_USE_CCS_BLOCK
void testbench::exec_stockham_dit( ac_int<32, false > xt[16384], ac_int<32, false > p,  ac_int<32, false > twiddle[16384],  ac_int<32, false > twiddle_h[16384]) {
   return mc_testbench::exec_stockham_dit(xt, p, twiddle, twiddle_h);
}
#endif
...
void mc_testbench::capture_xt_IN( ac_int<32, false > xt[16384])
{
   ...
   if (_capture_xt_IN && !testbench::xt_ignore)
   {
      ...
      mgc_sysc_ver_array1D<ac_int<32, false >,16384> xt_IN_tmp;
      int xt_linear_idx = 0;
      for (int xt_idx_0 = 0; xt_idx_0 < 16384; ++xt_idx_0)
...
void mc_testbench::capture_xt( ac_int<32, false > xt[16384])
{
   if (_capture_xt)
   {
      int cur_iter = xt_iteration_count;
      ++xt_iteration_count;
      mc_golden_info< mgc_sysc_ver_array1D<ac_int<32, false >,16384>, MaskPacket<0, 32> > xt_tmp(testbench::xt_ignore, false, xt_iteration_count);
      xt_tmp._data.mc_testbench_process_array_bounds("xt",testbench::xt_array_comp_first,testbench::xt_array_comp_last,16383,0);
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_xt_IN->ok_to_put() | ccs_p->ok_to_put() | ccs_twiddle->ok_to_put() | ccs_twiddle_h->ok_to_put());
      else
         wait(ccs_xt_IN->ok_to_put() | ccs_p->ok_to_put() | ccs_twiddle->ok_to_put() | ccs_twiddle_h->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN( ac_int<32, false > xt[16384], ac_int<32, false > p,  ac_int<32, false > twiddle[16384],  ac_int<32, false > twiddle_h[16384])
{
...
void mc_testbench::capture_OUT( ac_int<32, false > xt[16384], ac_int<32, false > p,  ac_int<32, false > twiddle[16384],  ac_int<32, false > twiddle_h[16384])
{
   that->capture_xt(xt);
}
// ============================================
...
void mc_testbench::exec_stockham_dit( ac_int<32, false > xt[16384], ac_int<32, false > p,  ac_int<32, false > twiddle[16384],  ac_int<32, false > twiddle_h[16384])
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(xt, p, twiddle, twiddle_h);
   stockham_dit(xt, p, twiddle, twiddle_h);
   ...
   capture_OUT(xt, p, twiddle, twiddle_h);
   #else
   #endif
}
// ============================================
